...
// 调用userModal组件中方法
const handleAddUserBtn = (data?: any) => {
  modalRef.value?.handleAddUserBtn(data)
}
...
const handleDeleteUser = async (userId: number) => {
  const deleteRes = await systemStore.deleteUser(userId)
  if (deleteRes.code === 0) {
    ElNotification({
      title: '删除成功',
...
const handlePagination = () => {
  ...
  systemStore.postUserList(offset, size)
}
</script>
...
<template>
  <div class="user-table">
    <div class="table-header">
      ...
      <el-button size="large" type="primary" @click="handleAddUserBtn()"
        >新建数据</el-button
      >
    </div>
        ...
        <el-table-column align="center" label="操作" width="170">
          <template #default="scope">
            <el-button
              ...
              size="small"
              text
              @click="handleAddUserBtn(scope.row)"
            >
              编辑
            </el-button>
            <el-popconfirm
